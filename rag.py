...
def classify_intent_by_vector(query: str, threshold: float = 0.5):
...
def classify_intent_by_llm(query: str) -> str:
    """2차: LLM으로 Intent 분류 (1차에서 확신도 낮을 때)"""
...
def classify_intent(query: str) -> str:
    """하이브리드 Intent 분류: 1차(벡터) → 2차(LLM)"""

    # 1차: 벡터 유사도로 빠르게 분류
    intent, distance = classify_intent_by_vector(query)
    print(f"1차 분류 결과: {intent} (거리: {distance:.2f})")

    # 1차에서 확신도 높으면 바로 반환
    if intent:
        return intent

    # 1차에서 애매하면 2차 LLM으로 정확히 분류
    print("1차 분류 불확실 → 2차 LLM 분류 시작")
    intent = classify_intent_by_llm(query)
    print(f"2차 분류 결과: {intent}")
